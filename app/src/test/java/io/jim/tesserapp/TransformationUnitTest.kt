...
class TransformationUnitTest {
    ...
    @Test
    fun rotation() {
        val m = Matrix(4).apply { rotation(1, 3, PI / 2) }
    ...
    @Test
    fun translation() {
        val m = Matrix(2).apply { translation(Vector(2.0, 1.0)) }
    ...
    fun spaces() {
        ...
        val s = Matrix(3).apply { scale(5.0) }
        val n = Matrix(3).apply {
            space(Vector(3),
                    Vector(5.0, 0.0, 0.0),
                    Vector(0.0, 5.0, 0.0),
                    Vector(0.0, 0.0, 5.0)
    ...
    @Test
    fun perspective() {
        assertEquals(2.0 / 10.0, (Vector(2.0, 3.0, -10.0) applyPoint Matrix(3).apply { perspective() }).x, 0.1)

        val m = Matrix(3).apply { perspective(5.0, 10.0) }
    ...
    fun combination() {
        val m = Matrix(3)
        m.multiplicationFrom(Matrix(3).apply { scale(2.0) }, Matrix(3).apply { translation(Vector(4.0, 0.0, 0.0)) })
